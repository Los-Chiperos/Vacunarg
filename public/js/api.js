...
document.getElementById("vaccineForm").addEventListener("submit", function (event) {
    ...
    fetch(apiUrl, {
        ...
        .then((data) => {
            ...
            let formattedFechaNacimiento = fechaNacimiento.getDate() + '/' + (fechaNacimiento.getMonth() + 1) + '/' + fechaNacimiento.getFullYear();

            let fechaAplicacion = new Date(paciente.fecha_aplicacion);
            let formattedFechaAplicacion = fechaAplicacion.getDate() + '/' + (fechaAplicacion.getMonth() + 1) + '/' + fechaAplicacion.getFullYear();
